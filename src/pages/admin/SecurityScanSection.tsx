    ...
    try {
      ...
      const vulns = Object.entries(vulnMap).map(([name, v]: [string, any]) => ({
        ...
        via: Array.isArray(v.via)
          ? v.via
              .map((x: any) => (typeof x === 'string' ? x : (x.name ?? x.title ?? '')))
              .filter(Boolean)
          : [],
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <p style={{ fontSize: 13, color: 'var(--color-text-muted)', flex: 1, margin: 0 }}>
          ...
          against all installed packages and reports known CVE vulnerabilities.
        </p>
        <button
          onClick={run}
          disabled={loading}
                ...
                >
                  <div
                    style={{
                      fontSize: 22,
                      fontWeight: 800,
                      color: SEV_COLOR[sev] ?? SEV_COLOR.info,
                    }}
                  >
                    {count as number}
                  </div>
                ...
                >
                  ...
                  >
                    ...
                    <ChevronRight
                      ...
                      style={{
                        ...
                        transform: expanded === v.name ? 'rotate(90deg)' : 'none',
                  ...
                  {expanded === v.name && (
                    ...
                    >
                      ...
                      {v.via.length > 0 && (
                        <div style={{ marginBottom: 6 }}>
                          <strong>Via:</strong> {v.via.join(', ')}
                        </div>
                      )}
                      <div>
                        <strong>Fix:</strong>{' '}
                        {v.fixAvailable === true ? (
                          <span style={{ color: '#059669' }}>Available (run npm audit fix)</span>
                        ) : typeof v.fixAvailable === 'object' ? (
                          <span style={{ color: '#d97706' }}>
                            Breaking fix via {v.fixAvailable.name}@{v.fixAvailable.version}
                          </span>
                        ) : (
...
function CodeScanTab({ authHeaders }: { authHeaders: () => Record<string, string> }) {
  ...
  for (const f of findings) {
    if (f.severity in bySev) (bySev as any)[f.severity]++
  }
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <p style={{ fontSize: 13, color: 'var(--color-text-muted)', flex: 1, margin: 0 }}>
          ...
          for hardcoded secrets, eval usage, SQL injection, XSS risks, and more.
        </p>
        <button
          onClick={run}
          disabled={loading}
...
function IntegrityTab({ authHeaders }: { authHeaders: () => Record<string, string> }) {
  ...
  const hasChanges =
    comparison &&
    comparison.added.length + comparison.removed.length + comparison.modified.length > 0

  return (
    <div>
      <p style={{ fontSize: 13, color: 'var(--color-text-muted)', margin: '0 0 20px' }}>
        Take a SHA-256 hash snapshot of the entire codebase as a baseline. Run again later to detect
        any modified, added, or removed files — useful for catching unauthorized changes or
        intrusions.
      </p>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      >
        ...
        <span>
          These tools are supplemental. They flag common patterns but are not a replacement for a
          full penetration test or professional security audit. Always review findings manually
