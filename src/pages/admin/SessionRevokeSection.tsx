...
export default function SessionRevokeSection({ authHeaders }: SectionProps) {
  ...
  async function submit() {
    ...
    if (scope === 'ip' && !ipPattern) {
      setErr('IP pattern required')
      return
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 540 }}>
      ...
      >
        ...
        This immediately logs out users. The <strong>All Sessions</strong> scope requires a second
        admin approval.
      </div>
          ...
          {(['user', 'ip', 'all'] as Scope[]).map(s => (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                border:
                  scope === s ? '1px solid var(--color-brand)' : '1px solid var(--color-border)',
