  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        {!loading && (
          <div style={{ display: 'flex', gap: 10, fontSize: 12 }}>
            ...
            {summary.degraded > 0 && (
              <span style={{ color: '#D97706', fontWeight: 600 }}>{summary.degraded} degraded</span>
            )}
            {summary.down > 0 && (
              <span style={{ color: '#DC2626', fontWeight: 600 }}>{summary.down} down</span>
            )}
          </div>
        )}
      ...
      {devEnv && !loading && (
        ...
        >
          Running in dev — Replit's network sandbox may block outbound pings to some domains.
          Results will be accurate on the deployed admin server.
        </div>
      )}
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--color-text-muted)', margin: 0 }}>
                {d.note}
                {d.error && <span style={{ color: '#DC2626', marginLeft: 8 }}>— {d.error}</span>}
              </p>
            </div>
