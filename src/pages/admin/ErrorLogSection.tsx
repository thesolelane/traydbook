...
export default function ErrorLogSection({ authHeaders }: SectionProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--color-border)', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {contexts.map(ctx => (
            <button
              key={ctx}
            ...
            {entries.map((entry, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Circle size={8} fill={color} color={color} style={{ marginTop: 5, flexShrink: 0 }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        <span style={{
                          fontSize: 10, fontWeight: 700, textTransform: 'uppercase',
                          letterSpacing: '0.5px', color, padding: '2px 6px',
                        ...
                        {entry.statusCode && (
                          <span style={{ fontSize: 11, color: entry.statusCode >= 500 ? '#e05252' : '#e0a03a', fontWeight: 700 }}>
                            {entry.statusCode}
                          </span>
                        )}
                      ...
                      <p style={{ margin: '4px 0 0', fontSize: 13, fontWeight: 600, color: 'var(--color-text)' }}>
                        {entry.message}
                      </p>
                    </div>
                    ...
                    <div style={{
                      ...
                      {entry.detail && (
                        <div style={{ marginTop: 12 }}>
                          <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text-muted)', margin: '0 0 4px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                            Detail
                          </p>
                      ...
                      {entry.userId && (
                        <div style={{ marginTop: 10 }}>
                          ...
                          <code style={{ fontSize: 12, color: 'var(--color-text)', fontFamily: 'monospace' }}>
                            {entry.userId}
                          </code>
                        </div>
                      )}
                      {entry.stack && (
                        <div style={{ marginTop: 10 }}>
                          ...
                          <pre style={{
                            fontSize: 11, color: '#e05252', fontFamily: 'monospace',
                            background: '#1a0a0a', padding: 12, borderRadius: 6,
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              ...
              <div key={ctx} style={{
                ...
                <p style={{ margin: '0 0 4px', fontSize: 13, fontWeight: 700, color: 'var(--color-text)' }}>{label}</p>
                <p style={{ margin: 0, fontSize: 12, color: 'var(--color-text-muted)', lineHeight: 1.5 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
