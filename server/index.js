...
/** Middleware: admin or admin_2 level required */
...
app.get('/api/admin/stats', requireAuth, requireAdminLevel, async (req, res) => {
  try {
    const [
      { count: totalUsers },
      { count: adminCount },
      ...
      supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).in('account_type', ['admin', 'admin_2']),
      supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).eq('account_type', 'contractor'),
      supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).in('account_type', ['project_owner', 'homeowner', 'agent']),
...
/** GET /api/admin/users — paginated user list with optional search/role filter */
app.get('/api/admin/users', requireAuth, requireAdminLevel, async (req, res) => {
  ...
  try {
    let q = supabaseAdmin
      .from('users')
      .select('id, display_name, handle, avatar_url, account_type, credit_balance, created_at, deleted_at')
      .order('created_at', { ascending: false })
...
/** PATCH /api/admin/user/:id/role — change a user's role */
app.patch('/api/admin/user/:id/role', requireAuth, requireAdminLevel, async (req, res) => {
  const { id } = req.params
  const { role } = req.body ?? {}
  if (!role || !ALL_INVITE_ROLES.includes(role)) return res.status(400).json({ error: 'Invalid role' })

  // Fetch the target user's current role to enforce authz boundaries
  const { data: targetUser, error: targetErr } = await supabaseAdmin
    .from('users').select('account_type').eq('id', id).single()
  ...
  // admin_2 cannot touch admin-level users in either direction
  ...
  if (error) return res.status(500).json({ error: error.message })
  console.log(`[admin/role] User ${id} role changed from ${targetUser.account_type} to ${role} by ${req.user.id}`)
  res.json({ ok: true })
})

/** PATCH /api/admin/user/:id/suspend — suspend or reinstate an account via deleted_at */
app.patch('/api/admin/user/:id/suspend', requireAuth, requireAdminLevel, async (req, res) => {
  const { id } = req.params
  ...
  // Fetch target user's current role to apply admin_2 boundary
  const { data: targetUser, error: targetErr } = await supabaseAdmin
    .from('users').select('account_type').eq('id', id).single()
  if (targetErr || !targetUser) return res.status(404).json({ error: 'Target user not found' })

  if (req.adminUser.account_type === 'admin_2' && ['admin', 'admin_2'].includes(targetUser.account_type)) {
    return res.status(403).json({ error: 'admin_2 cannot suspend or reinstate admin-level accounts' })
  }
  ...
  if (error) return res.status(500).json({ error: error.message })
  console.log(`[admin/suspend] User ${id} ${suspend ? 'suspended' : 'reinstated'} by ${req.user.id}`)
  res.json({ ok: true })
})

/** POST /api/admin/credits — manually adjust a user's credit balance */
app.post('/api/admin/credits', requireAuth, requireAdminLevel, async (req, res) => {
  ...
  const { error: updateErr } = await supabaseAdmin
    .from('users').update({ credit_balance: newBalance }).eq('id', userId)
  if (updateErr) return res.status(500).json({ error: updateErr.message })

  const transactionType = delta > 0 ? 'refund' : 'spend'
  ...
  console.log(`[admin/credits] Adjusted ${delta} credits for user ${userId} by admin ${req.user.id}. New balance: ${newBalance}`)
  res.json({ ok: true, balance: newBalance })
})

/** GET /api/admin/purchases — all purchases for admin payments view */
app.get('/api/admin/purchases', requireAuth, requireAdminLevel, async (req, res) => {
  const { status } = req.query
  try {
    let q = supabaseAdmin
      .from('purchases')
      .select('id, user_id, credits, amount_cents, status, created_at, users!user_id(display_name, handle)')
      .order('created_at', { ascending: false })
      .limit(100)
    ...
    const { data: allPurchases } = await supabaseAdmin.from('purchases').select('status, amount_cents')
...
/** GET /api/admin/posts — recent posts for moderation */
app.get('/api/admin/posts', requireAuth, requireAdminLevel, async (req, res) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('posts')
      .select('id, body, post_type, like_count, comment_count, created_at, is_flagged, author:users!author_id(display_name, handle)')
      .order('created_at', { ascending: false })
      .limit(30)
    if (error) return res.status(500).json({ error: error.message })
    res.json({ posts: data ?? [] })
...
app.get('/api/admin/comments', requireAuth, requireAdminLevel, async (req, res) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('comments')
...
app.delete('/api/admin/post/:id', requireAuth, requireAdminLevel, async (req, res) => {
  const { id } = req.params
...
/** PATCH /api/admin/post/:id/flag — flag or unflag a post for review */
app.patch('/api/admin/post/:id/flag', requireAuth, requireAdminLevel, async (req, res) => {
  const { id } = req.params
  const { flagged } = req.body ?? {}
  const { error } = await supabaseAdmin.from('posts').update({ is_flagged: !!flagged }).eq('id', id)
  if (error) return res.status(500).json({ error: error.message })
  console.log(`[admin/post] Post ${id} ${flagged ? 'flagged' : 'unflagged'} by admin ${req.user.id}`)
  res.json({ ok: true })
})

/** GET /api/admin/flagged-posts — posts flagged for review */
app.get('/api/admin/flagged-posts', requireAuth, requireAdminLevel, async (req, res) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('posts')
      .select('id, body, post_type, like_count, comment_count, created_at, is_flagged, author:users!author_id(display_name, handle)')
...
app.delete('/api/admin/comment/:id', requireAuth, requireAdminLevel, async (req, res) => {
  const { id } = req.params
...
/** GET /api/admin/wallets — users with wallet and credit info */
app.get('/api/admin/wallets', requireAuth, requireAdminLevel, async (req, res) => {
  try {
    ...
    // solana_wallets may not exist yet — tolerate table-not-found errors gracefully
    let wallets = []
    try {
