...
import { stripe, supabaseAdmin, STRIPE_WEBHOOK_SECRET, BUNDLES, APP_ORIGIN } from '../lib/clients.js'
...
async function handleSmsSubscriptionActivated(session, userId, smsTier) {
  ...
  const { error } = await supabaseAdmin
    .from('users')
    .update({ sms_tier: smsTier, sms_alerts_enabled: true, stripe_sms_sub_id: stripeSubscriptionId })
    .eq('id', userId)
  if (error) {
...
async function handleSmsSubscriptionCancelled(sub) {
  ...
  const { error } = await supabaseAdmin
    .from('users')
    .update({ sms_tier: null, sms_alerts_enabled: true, phone_verified: false, phone_number: null, sms_count_this_period: 0, stripe_sms_sub_id: null })
  ...
  async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (smsTier) {
        ...
        const { data: shouldCredit, error: fulfillErr } = await supabaseAdmin.rpc('fulfill_stripe_purchase', {
    ...
    if (event.type === 'customer.subscription.updated') {
      ...
      if (userId) {
        if (sub.status === 'active') {
          await supabaseAdmin.from('users').update({ sms_count_this_period: 0 }).eq('id', userId).eq('stripe_sms_sub_id', sub.id)
          ...
          await supabaseAdmin.from('users').update({ sms_alerts_enabled: false }).eq('id', userId).eq('stripe_sms_sub_id', sub.id)
    ...
    if (event.type === 'invoice.payment_succeeded') {
      ...
      if (subId && invoice.billing_reason === 'subscription_cycle') {
        const { data: users } = await supabaseAdmin.from('users').select('id').eq('stripe_sms_sub_id', subId)
        if (users && users.length > 0) {
          await supabaseAdmin.from('users').update({ sms_count_this_period: 0 }).eq('stripe_sms_sub_id', subId)
...
router.post('/api/create-checkout-session', requireAuth, async (req, res) => {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { userId, credits: String(bundle.credits), bundleId: bundle.id, priceId: bundle.priceId },
    })
...
router.get('/api/session-status', requireAuth, async (req, res) => {
  ...
  const { data: user } = await supabaseAdmin.from('users').select('credit_balance').eq('id', userId).single()
