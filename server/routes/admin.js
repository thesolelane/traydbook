...
router.get('/api/admin/stats', requireAuth, requireAdminLevel, async (req, res) => {
  try {
    ...
    res.json({ totalUsers: totalUsers ?? 0, adminCount: adminCount ?? 0, contractorCount: contractorCount ?? 0, ownerCount: ownerCount ?? 0, postCount: postCount ?? 0, jobCount: jobCount ?? 0, rfqCount: rfqCount ?? 0, bidCount: bidCount ?? 0, totalCreditsIssued, totalCreditSpent, recentSignups: recentSignups ?? 0 })
  } catch (err) {
...
router.patch('/api/admin/user/:id/role', requireAuth, requireAdminLevel, async (req, res) => {
  ...
  const { data: targetUser, error: targetErr } = await supabaseAdmin.from('users').select('account_type').eq('id', id).single()
...
router.patch('/api/admin/user/:id/suspend', requireAuth, requireAdminLevel, async (req, res) => {
  ...
  const { error } = await supabaseAdmin.from('users').update({ deleted_at: suspend ? new Date().toISOString() : null }).eq('id', id)
  if (error) return res.status(500).json({ error: error.message })
...
router.post('/api/admin/credits', requireAuth, requireAdminLevel, async (req, res) => {
  ...
  const { data: userRow, error: fetchErr } = await supabaseAdmin.from('users').select('credit_balance').eq('id', userId).single()
  if (fetchErr || !userRow) return res.status(404).json({ error: 'User not found' })

  const newBalance = userRow.credit_balance + delta
  const { error: updateErr } = await supabaseAdmin.from('users').update({ credit_balance: newBalance }).eq('id', userId)
  if (updateErr) return res.status(500).json({ error: updateErr.message })

  await supabaseAdmin.from('credit_ledger').insert({
    user_id: userId, delta, balance_after: newBalance,
